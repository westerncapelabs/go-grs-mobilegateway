...
function GoGRSMobilegateway() {
    ...
    self.make_quiz_response_state = function(state_name, next_state, response, correct){
        return function(){
            return new ChoiceState(
                state_name,
                next_state,
    ...
    self.build_quiz_states = function() {
        ...
        p_quizzes.add_callback(function(result) {
            if (result.objects.length === 0){
                ...
                // Build quiz states
                var quiz_prefix, questions, question, answers, answer, state_name, a_choices, next_state;
                for (var j = 0; j < result.objects.length; j++){
                    quiz = result.objects[j];
                    quiz_prefix = "quiz_" + quiz.id;
                    questions = quiz.q_quiz_id.sort(self.sort_by_dimension("id"));
                    for (var q = 0; q < questions.length; q++){
                        question = questions[q];
                        answers = question.question_id.sort(self.sort_by_dimension("id"));
                        ...
                        for (var a = 0; a < answers.length; a++){
                            ...
                            self.add_creator_unless_exists(
                                state_name + "_" + answer.id,
                                self.make_quiz_response_state(state_name + "_" + answer.id,
                                    next_state, answer.response, answer.correct)
                            );
                        }
                        self.add_creator_unless_exists(
                            state_name,
